...
var __defProp, __export, Build, SVG_NS, HTML_NS, PrimitiveType, NonPrimitiveType, TYPE_CONSTANT, VALUE_CONSTANT, SERIALIZED_PREFIX, reWireGetterSetter, getHostRef, registerHost, isMemberInElement, customError, consoleError, STENCIL_DEV_MODE, consoleDevError, consoleDevWarn, cmpModules, loadModule, styles, modeResolutionChain, needsScopedSSR, CONTENT_REF_ID, ORG_LOCATION_ID, SLOT_NODE_ID, TEXT_NODE_ID, COMMENT_NODE_ID, HYDRATE_ID, HYDRATED_STYLE_ID, HYDRATE_CHILD_ID, SLOT_FB_CSS, XLINK_NS, FORM_ASSOCIATED_CUSTOM_ELEMENT_CALLBACKS, win, H, plt, supportsShadow, supportsListenerOptions, promiseResolve, supportsConstructableStylesheets, supportsMutableAdoptedStyleSheets, queueCongestion, queuePending, queueDomReads, queueDomWrites, queueDomWritesLow, queueTask, consume, consumeTimeout, flush, nextTick, readTask, writeTask, getAssetPath, isDef, isComplexType, escapeRegExpSpecialCharacters, RemoteValue, result_exports, ok, err, unwrap, unwrapErr, globalStyleSheet, lowerPathParam, isDtsFile, isTsFile, isTsxFile, isJsxFile, isJsFile, updateFallbackSlotVisibility, getSlottedChildNodes, getSlotChildSiblings, isNodeLocatedInSlot, addSlotRelocateNode, getSlotName, patchPseudoShadowDom, patchCloneNode, patchSlotAppendChild, patchSlotRemoveChild, patchSlotPrepend, patchSlotAppend, patchSlotInsertAdjacentHTML, patchSlotInsertAdjacentText, patchInsertBefore, patchSlotInsertAdjacentElement, patchTextContent, patchChildSlotNodes, patchSlottedNode, patchNextSibling, patchNextElementSibling, patchPreviousSibling, patchPreviousElementSibling, patchParentNode, validElementPatches, validNodesPatches, i, createTime, uniqueTime, rootAppliedStyles, registerStyle, addStyle, attachStyles, getScopeId, convertScopedToShadow, hydrateScopedToShadow, h, newVNode, Host, isHost, vdomFnUtils, convertToPublic, convertToPrivate, validateInputProperties, initializeClientHydrate, clientHydrate, initializeDocumentHydrate, createSimpleVNode, addSlottedNodes, findCorrespondingNode, safeSelector, restoreSafeSelector, _polyfillHost, _polyfillSlotted, _polyfillHostContext, _parenSuffix, _cssColonHostRe, _cssColonHostContextRe, _cssColonSlottedRe, _polyfillHostNoCombinator, _polyfillHostNoCombinatorRe, _shadowDOMSelectorsRe, _safePartRe, _selectorReSuffix, _polyfillHostRe, createSupportsRuleRe, _colonSlottedRe, _colonHostRe, _colonHostContextRe, _commentRe, stripComments, _commentWithHashRe, extractCommentsWithHash, _ruleRe, _curlyRe, _selectorPartsRe, OPEN_CURLY, CLOSE_CURLY, BLOCK_PLACEHOLDER, processRules, escapeBlocks, insertPolyfillHostInCssText, convertColonRule, colonHostPartReplacer, convertColonHost, colonHostContextPartReplacer, convertColonSlotted, convertColonHostContext, convertShadowDOMSelectors, makeScopeMatcher, selectorNeedsScoping, injectScopingSelector, applySimpleSelectorScope, applyStrictSelectorScope, scopeSelector, scopeSelectors, scopeCssText, replaceShadowCssHost, expandPartSelectors, scopeCss, computeMode, getMode, parsePropertyValue, getElement, createEvent, emitEvent, setAccessor, parseClassListRegex, parseClassList, CAPTURE_EVENT_SUFFIX, CAPTURE_EVENT_REGEX, updateElement, scopeId, contentRef, hostTagName, useNativeShadowDom, checkSlotFallbackVisibility, checkSlotRelocate, isSvgMode, createElm, relocateToHostRoot, putBackInOriginalLocation, addVnodes, removeVnodes, updateChildren, isSameVnode, referenceNode, patch, relocateNodes, markSlotContentForRelocation, nullifyVNodeRefs, insertBefore, renderVdom, slotReferenceDebugNode, originalLocationDebugNode, attachToAncestor, scheduleUpdate, dispatchHooks, enqueue, isPromisey, updateComponent, renderingRef, callRender, postUpdateComponent, forceUpdate, appDidLoad, safeCall, emitLifecycleEvent, addHydratedFlag, serverSideConnected, getValue, setValue, proxyComponent, initializeComponent, fireConnectedCallback, connectedCallback, setContentReference, disconnectInstance, disconnectedCallback, proxyCustomElement, Fragment, addHostEventListeners, hostListenerProxy, getHostListenerTarget, hostListenerOpts, baseClass;
var init_client = __esm({
  "node_modules/@stencil/core/internal/client/index.js"() {
    ...
    STENCIL_DEV_MODE = BUILD.isTesting ? ["STENCIL:"] : [
      "%cstencil",
    ...
    loadModule = (cmpMeta, hostRef, hmrVersionId) => {
      ...
      return globImport_entry_js(`./${bundleId}.entry.js${BUILD.hotModuleReplacement && hmrVersionId ? "?s-hmr=" + hmrVersionId : ""}`).then(
        (importedModule) => {
    ...
    lowerPathParam = (fn) => (p) => fn(p.toLowerCase());
    isDtsFile = lowerPathParam((p) => p.endsWith(".d.ts") || p.endsWith(".d.mts") || p.endsWith(".d.cts"));
    isTsFile = lowerPathParam(
      (p) => !isDtsFile(p) && (p.endsWith(".ts") || p.endsWith(".mts") || p.endsWith(".cts"))
    );
    isTsxFile = lowerPathParam(
      (p) => p.endsWith(".tsx") || p.endsWith(".mtsx") || p.endsWith(".ctsx")
    );
    isJsxFile = lowerPathParam(
      (p) => p.endsWith(".jsx") || p.endsWith(".mjsx") || p.endsWith(".cjsx")
    );
    isJsFile = lowerPathParam((p) => p.endsWith(".js") || p.endsWith(".mjs") || p.endsWith(".cjs"));
...
More information: https://stenciljs.com/docs/properties#prop-mutability`
    ...
    baseClass = BUILD.lazyLoad ? class {
...
@stencil/core/internal/client/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-7FDHUQWW.js.map
